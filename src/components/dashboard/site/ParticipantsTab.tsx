...
// (App.tsx handleNavigateToParticipant); payloads older than the TTL are
// dropped so a stale handoff never auto-opens a drawer out of context.
const PENDING_PARTICIPANT_KEY = 'piq-pending-participant-v1';
...
export default function ParticipantsTab() {
  ...
  // Deep-link pickup — chat reference chips drop a participant payload
  // into `piq-pending-participant-v1` then navigate the user here. Once
  // the participants list is loaded, consume the key exactly once
  // (removed before any match attempt so a non-matching id can't linger
  // and fire days later), then auto-open the profile on a fresh match.
  useEffect(() => {
    if (loading || !participants.length) return;
    let raw: string | null = null;
    try {
      raw = localStorage.getItem(PENDING_PARTICIPANT_KEY);
      if (raw !== null) localStorage.removeItem(PENDING_PARTICIPANT_KEY);
    } catch {
      /* ignore */
    }
    if (!raw) return;
