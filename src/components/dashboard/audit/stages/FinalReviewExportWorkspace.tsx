...
function buildMarkdown(
  ...
  if (audit.scheduled_date) push(`**Scheduled:** ${formatAuditWindow(audit.scheduled_date, audit.scheduled_end_date)}  `);
...
async function buildDocx(
  ...
  const h1 = (text: string) => new Paragraph({ text, heading: HeadingLevel.HEADING_1 });
  const h2 = (text: string) => new Paragraph({ text, heading: HeadingLevel.HEADING_2 });
  const bold = (label: string, value: string) =>
    new Paragraph({ children: [new TextRun({ text: label, bold: true }), new TextRun({ text: value })] });
  const italic = (text: string) =>
    new Paragraph({ children: [new TextRun({ text, italics: true })] });

  const children: Paragraph[] = [];

  children.push(h1(audit.audit_name));
  children.push(bold('Vendor: ', audit.vendor_name));
  ...
  if (audit.scheduled_date) children.push(bold('Scheduled: ', formatAuditWindow(audit.scheduled_date, audit.scheduled_end_date) ?? ''));
  if (report.final_signed_off_at)
    children.push(bold('Signed off: ', formatTimestamp(report.final_signed_off_at)));

  children.push(blank(), h2('Executive Summary'), p(report.executive_summary));

  if (riskSummary) {
    children.push(blank(), h2('Risk Context'), p(riskSummary.vendor_relevance_narrative));
    if (riskSummary.focus_areas.length > 0)
  ...
  children.push(blank(), h2('Conclusions'), p(report.conclusions));
  children.push(
    blank(),
    italic('Generated by PIQClinical. Add sponsor branding, header/footer, and institutional sign-off before external distribution.'),
    italic(`Exported: ${new Date().toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`),
  );
