...
export const DELIVERABLE_EXPORT_DISCLAIMER =
  'This checklist was generated by PIQC as a source-backed draft preparation ' +
  'aid. Monitoring decisions, final content, and controlled release remain ' +
  "with your organization's designated process outside PIQC.";

export const DELIVERABLE_EXPORT_HEADER_LABEL =
  'PIQC drafted · Monitoring preparation checklist · DRAFT';
...
/**
 * Build the filename for an exported checklist:
 *   `<protocol_code-slug>_monitoring_prep_checklist_draft_<YYYY-MM-DD>.pdf`
 *
 * Protocol code falls back to the literal `protocol` when null (the export
 * packet carries no protocol_id to prefix with). Date comes from
 * `packet.generated_at` (server-stamped) NOT the browser clock — filename
 * and footer timestamp must agree.
 */
...
export function buildDeliverablePdf(packet: DeliverableExportPacket): jsPDF {
  ...
  doc.text(DELIVERABLE_EXPORT_HEADER_LABEL, margin, margin);
  ...
  for (const { sectionKey, blocks } of groupBlocksBySection(
    ...
    doc.setFontSize(11);
    doc.setTextColor(20);
    doc.text(MONITORING_SECTION_LABELS[sectionKey], margin, cursorY);
    cursorY += 4;
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    if (i > 1) {
      ...
      doc.text(DELIVERABLE_EXPORT_HEADER_LABEL, margin, margin - 12);
