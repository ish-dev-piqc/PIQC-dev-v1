-- =============================================================================
-- Protocol Deliverable Engine — deliverable_generate v3: prohibited-medication
-- facts in the monitoring checklist.
--
-- Extraction now supplies field_type = 'prohibited_med' rows (verbatim
-- medication/class entries from CLINICAL_EXTRACT_SCHEMA's
-- prohibited_medications field, evidence-linked like every other fact). This
-- migration re-issues deliverable_generate so checklist section 2
-- (exclusion_prohibited_med_review) consumes them:
--
--   1. One checklist_item fact card per prohibited_med row — evidence +
--      confidence passthrough, medication text VERBATIM from the fact pool
--      ("current_text wins"; PIQC never normalizes or invents drug names).
--   2. The coverage-gap framing block is DEMOTED to a fallback — it now
--      emits ONLY when zero prohibited_med rows exist (completeness
--      doctrine: absence of extraction ≠ absence of restrictions, so the
--      section still tells the CRA to verify manually instead of going
--      silent).
--
-- prohibited_med rows need no new plumbing here: the _deliv_items fact-pool
-- load has no field_type allowlist, so they flow through untouched, in
-- field_path order, with the same "current_text wins" / rejected_from_draft
-- contract as every other fact.
--
-- Everything else is byte-preserved from 20260709000200: the risk_overview
-- branch, all other checklist sections, the fingerprint + match + apply
-- regeneration stage, the per-type title CASE, and the helper calls. Prose
-- templates are byte-for-byte copies of the unit-tested TS spec in
-- src/lib/deliverables/selection/monitoringChecklist.ts (em dashes included)
-- — change the TS spec first, keep its tests green, then mirror here.
--
-- Draft-only vocabulary throughout — nothing here is ever "signed off as
-- final".
--
...
-- content).
--
-- Design + decisions: plans/fable/prohibited-meds-extraction.md.
-- =============================================================================
...
BEGIN
  ...
  -- apply) is artifact-agnostic. The checklist branch preserves the
  -- 20260709000200 emission stage byte-for-byte except section 2, which now
  -- emits prohibited_med fact cards and demotes the coverage-gap block to a
  -- zero-rows fallback — original indentation kept on purpose so versions
  -- stay mechanically diffable.
  -- =========================================================================
