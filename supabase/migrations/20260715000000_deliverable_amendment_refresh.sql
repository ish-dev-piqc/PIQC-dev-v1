-- =============================================================================
-- Amendment-Aware Refresh (Phase 5) — change visibility after regenerate.
--
-- The engine's regenerate already preserves human work; what it could not do
-- was TELL ITS STORY: pristine deletions vanished silently and new blocks
-- looked like old ones. This migration makes every generate auditable:
--
--   1. generation_seq on protocol_deliverables (incremented per generate)
--      and on protocol_deliverable_blocks (birth generation — matched blocks
--      keep theirs). "New since the previous generation" is a pure
--      seq-equality test.
--   2. deliverable_generation_log — append-only, one row per generate, with
--      counters and removed_blocks snapshots ({section_key, block_type,
--      derived_text}) of the pristine drafts the run deleted. Only pristine
--      drafts are ever deleted, so nothing human-authored or human-reviewed
--      exists solely in the log.
--   3. deliverable_generate v6 — seq bump + stamping + log row. ALL FOUR
--      artifact branches are byte-preserved from 20260713000000; the changes
--      live in the shared upsert/apply stages only.
--   4. deliverable_get_packet v2 + _deliv_blocks_json v2 — packets carry the
--      deliverable and per-block generation_seq.
--   5. deliverable_get_change_summary — the reviewer's amendment lens:
--      latest log row + new / flagged / removed lists.
--
-- The generate RPC's RETURN shape is UNCHANGED (client contract stable);
-- clients read change data through the packet + summary RPC.
--
-- TS mirrors: src/types/deliverables/index.ts (DeliverableGenerationLog,
-- DeliverableChangeSummary, generation_seq fields).
-- Design + decisions: plans/fable/amendment-refresh.md.
-- =============================================================================

ALTER TABLE public.protocol_deliverables
  ADD COLUMN IF NOT EXISTS generation_seq INTEGER NOT NULL DEFAULT 1;

ALTER TABLE public.protocol_deliverable_blocks
  ADD COLUMN IF NOT EXISTS generation_seq INTEGER NOT NULL DEFAULT 1;

COMMENT ON COLUMN public.protocol_deliverables.generation_seq IS
  'Incremented by every deliverable_generate run. Blocks born in the latest '
  'run carry the same value — the "new since previous generation" signal.';

COMMENT ON COLUMN public.protocol_deliverable_blocks.generation_seq IS
  'The deliverable generation this block was born in; matched blocks keep '
  'their birth seq across regenerates.';


-- ---------------------------------------------------------------------------
-- deliverable_generation_log — append-only run log (RLS mirrors the edits
-- table: SELECT + INSERT only; absence of UPDATE/DELETE policies is the
-- append-only enforcement).
-- ---------------------------------------------------------------------------

CREATE TABLE public.deliverable_generation_log (
  ...
  blocks_matched      INTEGER     NOT NULL DEFAULT 0,
  blocks_kept_flagged INTEGER     NOT NULL DEFAULT 0,
  blocks_deleted      INTEGER     NOT NULL DEFAULT 0,
  -- Snapshots of the pristine drafts this run deleted — the only record
  -- they existed. {section_key, block_type, derived_text} only.
  removed_blocks      JSONB       NOT NULL DEFAULT '[]'::jsonb
);

CREATE INDEX deliverable_generation_log_deliverable_idx
  ON public.deliverable_generation_log (deliverable_id, generation_seq DESC);

ALTER TABLE public.deliverable_generation_log ENABLE ROW LEVEL SECURITY;

CREATE POLICY "deliverable_generation_log_select"
  ON public.deliverable_generation_log FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.protocol_deliverables d
     WHERE d.id = deliverable_id
       AND public.user_can_access_protocol(auth.uid(), d.protocol_id)
  ));

CREATE POLICY "deliverable_generation_log_insert"
  ON public.deliverable_generation_log FOR INSERT TO authenticated
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.protocol_deliverables d
     WHERE d.id = deliverable_id
       AND public.user_can_access_protocol(auth.uid(), d.protocol_id)
...
COMMENT ON TABLE public.deliverable_generation_log IS
  'Append-only: one row per deliverable_generate run. removed_blocks holds '
  'snapshots of pristine drafts the run deleted (touched blocks are never '
  'deleted, so nothing human lives only here). TS mirror: '
  'DeliverableGenerationLog in src/types/deliverables/index.ts.';
...
-- ---------------------------------------------------------------------------
-- _deliv_blocks_json v2 + deliverable_get_packet v2 — packets carry the
-- generation_seq fields. Everything else byte-preserved from 20260708000100.
-- ---------------------------------------------------------------------------
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'generation_seq',     b.generation_seq
      )
      ORDER BY b.sort_order, b.created_at, b.id
    ),
    '[]'::jsonb
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'generation_seq',   v_deliv.generation_seq,
...
-- ---------------------------------------------------------------------------
-- deliverable_get_change_summary — the reviewer's amendment lens. INVOKER:
-- RLS scopes everything; an invisible deliverable returns NULL (no
-- existence leak, same contract as get_packet).
-- ---------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION deliverable_get_change_summary(
...
DECLARE
  v_user  UUID := auth.uid();
  v_deliv protocol_deliverables;
  v_log   deliverable_generation_log;
BEGIN
  IF v_user IS NULL THEN
    RAISE EXCEPTION 'Not authenticated' USING ERRCODE = '42501';
  END IF;

  SELECT * INTO v_deliv FROM protocol_deliverables WHERE id = p_deliverable_id;
  IF NOT FOUND THEN
    RETURN NULL;
  END IF;

  SELECT * INTO v_log
    FROM deliverable_generation_log
   WHERE deliverable_id = p_deliverable_id
   ORDER BY generation_seq DESC, generated_at DESC
   LIMIT 1;

  RETURN jsonb_build_object(
    'log', CASE WHEN v_log.id IS NULL THEN NULL ELSE jsonb_build_object(
      'id',                  v_log.id,
      'deliverable_id',      v_log.deliverable_id,
      'generation_seq',      v_log.generation_seq,
      'protocol_version',    v_log.protocol_version,
      'generated_by',        v_log.generated_by,
      'generated_at',        v_log.generated_at,
      'blocks_created',      v_log.blocks_created,
      'blocks_matched',      v_log.blocks_matched,
      'blocks_kept_flagged', v_log.blocks_kept_flagged,
      'blocks_deleted',      v_log.blocks_deleted,
      'removed_blocks',      v_log.removed_blocks
    ) END,
    -- New in the latest generation — only meaningful after a regenerate
    -- (the first generation is birth, not change).
    'new_blocks', CASE WHEN v_deliv.generation_seq > 1 THEN (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
               'id',           b.id,
               'section_key',  b.section_key,
               'display_text', COALESCE(b.current_text, b.derived_text, '')
        ...
        FROM protocol_deliverable_blocks b
       WHERE b.deliverable_id = p_deliverable_id
         AND b.generation_seq = v_deliv.generation_seq
         AND b.review_state <> 'rejected'
    ) ELSE '[]'::jsonb END,
    'flagged_blocks', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        FROM protocol_deliverable_blocks b
       WHERE b.deliverable_id = p_deliverable_id
         AND b.review_state = 'needs_review'
    ),
    'removed_blocks', COALESCE(v_log.removed_blocks, '[]'::jsonb)
  );
END;
$$;

COMMENT ON FUNCTION deliverable_get_change_summary IS
  'The reviewer''s amendment lens: latest generation-log row + the lists a '
  'reviewer needs after a regenerate — new blocks (birth seq = current seq, '
  'seq > 1), currently-flagged blocks (needs_review: regenerate-orphaned and '
  'human-flagged share one queue by design), and the log''s removed-block '
  'snapshots. NULL for an invisible/absent deliverable (no existence leak). '
  'SECURITY INVOKER.';

GRANT EXECUTE ON FUNCTION deliverable_get_change_summary(UUID) TO authenticated;
