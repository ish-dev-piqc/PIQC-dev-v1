-- =============================================================================
-- Site Training Priorities (5th deliverable) — deliverable_generate v7.
--
-- Adds the site_training_priorities artifact type: what the SITE must be
-- trained on before activation, in an instructional register, selected from
-- facts already extracted (inclusion/exclusion criteria, the prohibited_med
-- facts from 20260710000000, endpoints, visits + procedures, amendment). Zero
-- new tables, zero new block types — the fifth lens over the one fact pool,
-- proving "parse once, generate many".
--
-- The four existing artifact branches (checklist / risk_overview /
-- cra_monitoring_focus / siv_package) AND the shared fingerprint/match/apply/
-- log stage are byte-preserved from 20260715000000 (v6 → v7). Only three
-- things change in the function: the dispatch guard list, the v_title CASE, and
-- the new site_training_priorities ELSIF branch. The RETURN shape is unchanged
-- (client contract stable).
--
-- ENUM HAZARD: ALTER TYPE ... ADD VALUE inside this transaction is safe here
-- because the value is only referenced inside the CREATE OR REPLACE FUNCTION
-- body (stored text, not evaluated until a later call commits) — the same
-- recipe as risk_overview / cra_monitoring_focus / siv_package.
--
-- TS spec (byte-parity by test): src/lib/deliverables/selection/siteTrainingPriorities.ts.
-- TS mirror: src/types/deliverables/index.ts (SiteTrainingSectionKey, labels).
-- Design + decisions: plans/fable/site-training-priorities.md.
-- =============================================================================

ALTER TYPE deliverable_artifact_type ADD VALUE IF NOT EXISTS 'site_training_priorities';
...
DECLARE
  ...
  -- v6 (amendment refresh): generation bookkeeping. The RETURN shape is
  -- unchanged — these feed the packet fields and the generation log.
  v_generation_seq   INTEGER := 1;
  v_matched          INTEGER := 0;
  v_kept_flagged     INTEGER := 0;
...
BEGIN
  ...
  IF p_artifact_type NOT IN
     ('monitoring_prep_checklist', 'risk_overview', 'cra_monitoring_focus',
      'siv_package', 'site_training_priorities') THEN
  ...
  v_title := CASE p_artifact_type
    ...
    WHEN 'site_training_priorities'  THEN 'Site Training Priorities — Draft'
  END;
  ...
  IF FOUND THEN
    UPDATE protocol_deliverables
     ...
     RETURNING generation_seq INTO v_generation_seq;
  ELSE
    ...
    ON CONFLICT (protocol_id, artifact_type) DO UPDATE
      SET title            = EXCLUDED.title,
          ...
          regenerated_at   = NOW(),
          generation_seq   = protocol_deliverables.generation_seq + 1
    RETURNING id, generation_seq INTO v_deliverable_id, v_generation_seq;
  END IF;
  ...
  ELSIF p_artifact_type = 'site_training_priorities' THEN

  -- =========================================================================
  -- THE SITE-TRAINING RULESET — port of selectSiteTrainingPrioritiesBlocks()
  -- in src/lib/deliverables/selection/siteTrainingPriorities.ts (the
  -- unit-tested spec — change rules there first, keep its tests green, then
  -- mirror here). Instructional register ("Train …", "Brief …", "Retrain …")
  -- — NEVER the checklist's "Verify:" / "Confirm absence of", the risk lens's
  -- "PIQC flagged", or the CRA lens's "Prioritize verification of". No numeric
  -- readiness scores anywhere. COMPLETENESS DOCTRINE: the four fact domains
  -- (eligibility, visits, procedures, endpoints) ALWAYS say something — facts
  -- when supported, an explicit "train from the protocol manually" gap block
  -- otherwise; safety + logistics questions are universal and always emit as
  -- framing. Prose is byte-for-byte the TS spec. The vendor/imaging/specimen
  -- taxonomy is the shared _deliv_procedure_category() (one list, never two).
  -- sort_order (v_sort) is one global 0-based counter.
  -- =========================================================================

  -- --- 1. eligibility_screening_training (facts; ALWAYS emits) --------------
  INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
  VALUES (v_sort, 'eligibility_screening_training', 'section_intro', 'derived_operational_framing',
  ...
  LOOP
    ...
    VALUES (
      v_sort, 'eligibility_screening_training', 'checklist_item', 'protocol_fact',
      'Train the screening team to confirm the inclusion requirement: ' || v_item.value_text,
      v_item.item_id, v_item.evidence_id, v_item.quoted_text,
      v_item.page_number, v_item.section_title,
      COALESCE(v_item.confidence_state, 'needs_review')
    );
  ...
  LOOP
    ...
    VALUES (
      v_sort, 'eligibility_screening_training', 'checklist_item', 'protocol_fact',
      'Train the screening team to rule out the exclusion: ' || v_item.value_text,
      v_item.item_id, v_item.evidence_id, v_item.quoted_text,
      v_item.page_number, v_item.section_title,
      COALESCE(v_item.confidence_state, 'needs_review')
    );
  ...
  LOOP
    ...
    VALUES (
      v_sort, 'eligibility_screening_training', 'checklist_item', 'protocol_fact',
      'Train staff to screen medication history for the restricted medication: ' || v_item.value_text,
      v_item.item_id, v_item.evidence_id, v_item.quoted_text,
      v_item.page_number, v_item.section_title,
      COALESCE(v_item.confidence_state, 'needs_review')
    );
  ...
  IF NOT EXISTS (
    SELECT 1 FROM _deliv_items
     WHERE field_type IN ('inclusion_criterion', 'exclusion_criterion', 'prohibited_med')
       AND value_text IS NOT NULL
  ) THEN
    INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
    VALUES (v_sort, 'eligibility_screening_training', 'checklist_item', 'derived_operational_framing',
  ...
  IF EXISTS (SELECT 1 FROM _deliv_visits) THEN
    INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
    VALUES (v_sort, 'visit_schedule_training', 'section_intro', 'derived_operational_framing',
    ...
    FOR v_visit IN SELECT * FROM _deliv_visits ORDER BY ord LOOP
      ...
      v_window_text := CASE
        WHEN v_visit.wminus = 0 AND v_visit.wplus = 0
          THEN 'no scheduling window — exact day'
        ELSE 'window −' || v_visit.wminus_txt || '/+' || v_visit.wplus_txt || ' days'
      END;
      ...
      VALUES (
        v_sort, 'visit_schedule_training', 'checklist_item', 'protocol_fact',
        'Brief coordinators on ' || v_visit.visit_name || ' (' || v_day_text
          || ', ' || v_window_text || ').'
          || CASE WHEN v_visit.wminus + v_visit.wplus <= 2
               THEN ' This is a narrow window — a scheduling slip becomes a protocol deviation, so '
  ...
  ELSE
    INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
    VALUES (v_sort, 'visit_schedule_training', 'checklist_item', 'derived_operational_framing',
  ...
  --     says something) — collect first so the intro can state the count -----
  CREATE TEMP TABLE IF NOT EXISTS _deliv_train_procs (
    seq            INTEGER,
    item_id        UUID,
    evidence_id    UUID,
    quoted_text    TEXT,   -- sensitive — never log
  ...
  DELETE FROM _deliv_train_procs;
  ...
  FOR v_visit IN SELECT * FROM _deliv_visits ORDER BY ord LOOP
    FOREACH v_proc IN ARRAY v_visit.procedures LOOP
      ...
      v_dep_total := v_dep_total + 1;
      INSERT INTO _deliv_train_procs (
        seq, item_id, evidence_id, quoted_text, page_number, section_title,
        visit_name, procedure_name, category
      )
      VALUES (
  ...
  IF v_dep_total > 0 THEN
    INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
    VALUES (v_sort, 'procedure_specimen_training', 'section_intro', 'derived_operational_framing',
      'The site will execute ' || v_dep_total || ' vendor, laboratory, imaging, or specimen '
      || CASE WHEN v_dep_total = 1 THEN 'workflow' ELSE 'workflows' END
      || ' below (keyword-based detection, so '
    ...
    FOR v_dep IN SELECT * FROM _deliv_train_procs ORDER BY seq LOOP
      INSERT INTO _deliv_new_specs (
        ord, section_key, block_type, content_origin, derived_text,
        extracted_item_id, source_evidence_id, source_quote,
        source_page_number, source_section, confidence_state
      ...
      VALUES (
        v_sort, 'procedure_specimen_training', 'checklist_item', 'protocol_fact',
        'Train site staff on ''' || v_dep.procedure_name || ''' (' || v_dep.visit_name
        || ') — ' || v_dep.category || ' '
  ...
  ELSE
    INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
    VALUES (v_sort, 'procedure_specimen_training', 'checklist_item', 'derived_operational_framing',
  ...
  IF EXISTS (
    ...
    INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
    VALUES (v_sort, 'endpoint_data_training', 'section_intro', 'derived_operational_framing',
    ...
    LOOP
      v_prefix := CASE
        WHEN v_item.field_path LIKE 'primary\_endpoints%'
          THEN 'PRIMARY ENDPOINT — train staff on source-data capture and documentation for: '
        ELSE 'Secondary endpoint — train staff on source-data capture for: '
      END;
      ...
      VALUES (
        v_sort, 'endpoint_data_training', 'checklist_item', 'protocol_fact',
        v_prefix || v_item.value_text,
        v_item.item_id, v_item.evidence_id, v_item.quoted_text,
        v_item.page_number, v_item.section_title,
        COALESCE(v_item.confidence_state, 'needs_review')
      );
  ...
  ELSE
    INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
    VALUES (v_sort, 'endpoint_data_training', 'checklist_item', 'derived_operational_framing',
  ...
  INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
  VALUES (v_sort, 'safety_oversight_training', 'section_intro', 'derived_operational_framing',
  ...
  INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
  VALUES (v_sort, 'safety_oversight_training', 'checklist_item', 'derived_operational_framing',
  ...
  -- --- 6. amendment_retraining (fact when an amendment was extracted) -------
  SELECT *
    INTO v_item
    FROM _deliv_items
   WHERE field_type = 'metadata'
  ...
  IF FOUND THEN
    INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
    VALUES (v_sort, 'amendment_retraining', 'section_intro', 'derived_operational_framing',
    ...
    VALUES (
      v_sort, 'amendment_retraining', 'checklist_item', 'protocol_fact',
      'Amendment noted: ' || v_item.value_text || '. Retrain affected site staff on the '
  ...
  ELSE
    INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
    VALUES (v_sort, 'amendment_retraining', 'checklist_item', 'derived_operational_framing',
  ...
  INSERT INTO _deliv_new_specs (ord, section_key, block_type, content_origin, derived_text)
  VALUES
    (v_sort, 'training_logistics_questions', 'site_question', 'derived_operational_framing',
      'Is the delegation-of-authority log current, with training documented for '
      'every delegated task before it is performed?'),
    (v_sort + 1, 'training_logistics_questions', 'site_question', 'derived_operational_framing',
    ...
    (v_sort + 2, 'training_logistics_questions', 'site_question', 'derived_operational_framing',
  ...
  -- v6: snapshot the pristine drafts about to be deleted — the generation
  -- log is the only record they existed (same predicate as the DELETE).
  ...
  GET DIAGNOSTICS v_kept_flagged = ROW_COUNT;
  ...
  -- v6: the run's change story, append-only. The RETURN below is unchanged.
  INSERT INTO deliverable_generation_log (
    deliverable_id, generation_seq, protocol_version, generated_by,
    blocks_created, blocks_matched, blocks_kept_flagged, blocks_deleted,
    removed_blocks
  )
  VALUES (
    v_deliverable_id, v_generation_seq, v_protocol_version, v_user,
    v_created, v_matched, v_kept_flagged, v_deleted, v_removed
  );
...
COMMENT ON FUNCTION deliverable_generate IS
  'Generates/regenerates a protocol deliverable. Five artifact types share '
  ...
  'sponsor-confirmation warning), and site_training_priorities (instructional '
  'site-readiness training, completeness doctrine — the four fact domains '
  'always say something). Per-type titles; regenerate preserves human work '
  '(human_editorial and current_text blocks survive; rejected blocks are '
