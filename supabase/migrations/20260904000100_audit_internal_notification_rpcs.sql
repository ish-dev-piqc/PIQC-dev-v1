-- =============================================================================
-- Audit Mode — internal audit notification: RPC slice (PR-D1)
--
-- Companion to 20260904000000 (which added the enum value + table). Split so
-- the new tracked_object_type value is never referenced in the transaction
-- that added it (20260707000200 precedent).
--
-- Clones the confirmation-letter lifecycle exactly:
--   - upsert:  demote-on-edit (content change → DRAFT, approval cleared),
--     readable 'AUDIT'-family deltas via audit_mode_write_delta
--     (20260430170000 shape)
--   - approve: CAS on updated_at with MISSING_EXPECTED_VERSION / STALE_CONTENT
--     hints (20260730000000 shape — the current letter approve, not phase-1's)
--   - apply-generation: content through the upsert (latch reused, never
--     duplicated), provenance stamped in the same transaction
--     (20260901000000 shape)
--
-- Plus the audit_mode_can_view_tracked_object full replacement adding the
-- INTERNAL_NOTIFICATION_OBJECT branch (keeps every branch from its latest
-- version in 20260725000100). Behavior for existing types is unchanged.
-- =============================================================================


-- -----------------------------------------------------------------------------
-- audit_mode_can_view_tracked_object — full replacement adding the
-- INTERNAL_NOTIFICATION_OBJECT branch.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION audit_mode_upsert_internal_notification(
  p_audit_id uuid,
  p_content  jsonb,
  p_reason   text DEFAULT NULL
)
RETURNS internal_notification_objects
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = public
AS $$
DECLARE
  ...
  v_before          internal_notification_objects;
  v_after           internal_notification_objects;
  v_diff            jsonb;
...
BEGIN
  ...
  SELECT * INTO v_before FROM internal_notification_objects WHERE audit_id = p_audit_id;

  IF NOT FOUND THEN
    INSERT INTO internal_notification_objects (audit_id, content, approval_status)
    VALUES (p_audit_id, p_content, 'DRAFT')
    RETURNING * INTO v_after;

    PERFORM audit_mode_write_delta(
      'INTERNAL_NOTIFICATION_OBJECT'::tracked_object_type,
      v_after.id,
      jsonb_build_object(
      ...
      v_user,
      COALESCE(p_reason, 'Internal notification created')
    );
    RETURN v_after;
  END IF;
  ...
  UPDATE internal_notification_objects SET
    content         = p_content,
  ...
  PERFORM audit_mode_write_delta(
    ...
    v_diff,
    v_user,
    COALESCE(p_reason, CASE WHEN v_content_changed THEN 'Internal notification edited (auto-demoted to DRAFT)' ELSE NULL END)
  );
...
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION audit_mode_approve_internal_notification(
  p_id                  uuid,
  p_reason              text        DEFAULT NULL,
...
BEGIN
  ...
  SELECT * INTO v_before FROM internal_notification_objects WHERE id = p_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Internal notification % not found', p_id USING ERRCODE = 'P0002';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Internal notification changed since it was last reviewed'
  ...
  PERFORM audit_mode_write_delta(
    ...
    v_diff,
    v_user,
    COALESCE(p_reason, 'Internal notification approved')
  );
...
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION audit_mode_apply_internal_notification_generation(
  p_audit_id           uuid,
  p_content            jsonb,
...
BEGIN
  ...
  v_after := audit_mode_upsert_internal_notification(
    p_audit_id,
    p_content,
    COALESCE(p_reason, 'Internal notification drafted by PIQC from protocol + evidence')
  );
...
-- Grants
-- -----------------------------------------------------------------------------
GRANT EXECUTE ON FUNCTION audit_mode_upsert_internal_notification(uuid, jsonb, text)                          TO authenticated;
GRANT EXECUTE ON FUNCTION audit_mode_approve_internal_notification(uuid, text, timestamptz)                   TO authenticated;
GRANT EXECUTE ON FUNCTION audit_mode_apply_internal_notification_generation(uuid, jsonb, jsonb, jsonb, text)  TO authenticated;
