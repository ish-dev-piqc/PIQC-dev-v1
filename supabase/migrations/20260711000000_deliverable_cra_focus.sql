-- =============================================================================
-- Protocol Deliverable Engine — third artifact type: cra_monitoring_focus,
-- plus the risk-lens prohibited-med debt.
--
-- "Parse once, generate many", third lens: the CRA Monitoring Focus is a
-- protocol-derived verification-emphasis view for monitors — where should a
-- CRA's limited on-site attention go FIRST — over the SAME fact pool, tables,
-- and review machinery as the other two artifacts. ZERO new tables, zero new
-- block_types.
--
--   1. deliverable_artifact_type += 'cra_monitoring_focus'.
--   2. deliverable_generate REPLACED (v4):
--      a. the per-type title CASE gains 'CRA Monitoring Focus — Draft
--         Preparation Aid';
--      b. the spec-emission dispatch gains the cra_monitoring_focus branch —
--         the PL/pgSQL port of the unit-tested spec in
--         src/lib/deliverables/selection/craMonitoringFocus.ts. Change the
--         TS spec first, keep its tests green, then mirror here;
--      c. the risk_overview branch settles the debt named in
--         plans/fable/prohibited-meds-extraction.md: Section 1
--         (eligibility_complexity) now emits one fact card per prohibited_med
--         row ("Restricted medication in eligibility scope: ...") with
--         evidence + confidence passthrough, and the section intro gains
--         medication-aware variants — mirrored from the updated
--         selection/riskOverview.ts;
--      d. the monitoring_prep_checklist branch, the fingerprint + match +
--         apply regeneration stage, and every helper call are byte-preserved
--         from 20260710000000.
--
-- Distinctiveness rule (plan Decision 3): checklist = imperative task cards
-- ("Verify: ..."); risk overview = fragility factors ("where execution is
-- fragile"); CRA focus = ATTENTION ALLOCATION ("Prioritize verification of
-- ...", "Plan on-site time for ..."). Same facts, different question — card
-- prose is never duplicated across lenses.
--
-- Doctrine: explainable prose ONLY — NO numeric risk scores anywhere.
-- Protocol-only (plan Decision 2): no site or participant context in any
-- prose. Medication text is VERBATIM from the fact pool ("current_text wins";
-- PIQC never normalizes or invents drug names). Draft-only vocabulary
...
-- Design + decisions: plans/fable/cra-monitoring-focus.md.
-- =============================================================================
...
-- this migration casts to or inserts 'cra_monitoring_focus'. Do NOT add any
-- immediate cast / INSERT / seed using the new value to this file.
-- ---------------------------------------------------------------------------

ALTER TYPE deliverable_artifact_type ADD VALUE IF NOT EXISTS 'cra_monitoring_focus';
...
--      'Protocol Risk Overview' / 'CRA Monitoring Focus — Draft
--      Preparation Aid') via CASE on p_artifact_type;
...
BEGIN
  ...
  IF p_artifact_type NOT IN
     ('monitoring_prep_checklist', 'risk_overview', 'cra_monitoring_focus') THEN
  ...
  -- =========================================================================
  -- SPEC-EMISSION DISPATCH — the only per-type stage. All three branches
...
COMMENT ON FUNCTION deliverable_generate IS
  ...
  'spec: src/lib/deliverables/selection/monitoringChecklist.ts), '
  'risk_overview (title "Protocol Risk Overview", spec: src/lib/'
  'deliverables/selection/riskOverview.ts), and cra_monitoring_focus '
  '(title "CRA Monitoring Focus — Draft Preparation Aid", spec: src/lib/'
  'deliverables/selection/craMonitoringFocus.ts — attention-allocation '
  'prose that never duplicates the other lenses). Explainable prose only, '
