/*
  # Fix hybrid_search to accept float8[] instead of vector

  PostgREST cannot auto-cast a JSON number array to the pgvector `vector` type.
  This migration replaces the function signature to accept `float8[]` and casts
  it to `vector` internally, so supabase.rpc() calls with a plain JS number array work correctly.
*/
