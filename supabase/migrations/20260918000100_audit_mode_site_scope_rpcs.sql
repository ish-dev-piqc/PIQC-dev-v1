-- =============================================================================
-- Audit Mode — ISA site audit scope: RPC slice (isa-scope-builder)
--
-- Companion to 20260918000000 (enum value + table). Split so the new
-- tracked_object_type value is never referenced in the transaction that
-- added it (20260707000200 precedent).
--
-- NO NEW FUNCTIONS. The scope is served by the generic deliverable pair
...
--
--   1. audit_mode_can_view_tracked_object — adds the SITE_SCOPE_OBJECT
--      branch (every branch from 20260917000100 kept). The
--      state_history_deltas INSERT policy runs it, so the branch must exist
--      before the first upsert writes a delta.
--
--   2. audit_mode_deliverable_kind_config — 8th arm, 'site_scope' →
--      site_scope_objects / SITE_SCOPE_OBJECT / 'Site audit scope' /
--      o_basis NULL (every arm from 20260907000100 kept). NULL basis: the
--      approve arm CAS-pins the row's own updated_at and refuses a digest
--      for this kind (22023) — the standard latch of the letter / agenda /
--      checklist kinds. The scope's derivation basis (the mapping set) is
--      recorded in content.built_from and checked client-side; a server pin
--      ('MAPPING_SET', a third basis token) is ledgered in
...
-- src/types/audit/objects.ts (SiteScopeContent) change in the same PR.
--
-- Owner: @rv61.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. audit_mode_can_view_tracked_object — adds SITE_SCOPE_OBJECT
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2. audit_mode_deliverable_kind_config — 8th arm. Dynamic SQL in the generic
