// =============================================================================
// Source of Truth Reviewer — Reducto → SOTR adapter (DENO COPY).
//
// This file is a literal copy of src/lib/sotr/sourceEvidenceAdapter.ts with
// adjusted import paths so the Supabase ingest edge function can run it on
// Deno. The canonical version lives in src/lib/sotr/; keep both in sync.
//
// When this file diverges from the canonical adapter:
//   1. Tests in src/lib/sotr/__tests__/ keep passing while production
//      behavior changes silently.
//   2. The frontend SOTR drawer renders different data than what was
//      written at ingest time.
// Always update both. Diff the two files before committing changes to either.
//
// Design principles (same as canonical):
