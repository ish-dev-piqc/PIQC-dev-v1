...
- @karl-dev-piqc — for src/lib/audit/__tests__/chatApi.test.ts

## Verification

How to test end-to-end. Filled in before opening the PR.

- [ ] `npx vitest run` on the 8 target files: all previously-failing tests pass
- [ ] Each fixed assertion re-checked against the current implementation (not loosened blindly — exportApi regex still rejects approval/signature language outside the disclaimer sentence)
- [ ] Full `npx vitest run`: only the pre-existing "supabaseUrl is required" collection failures remain (separate env concern)
